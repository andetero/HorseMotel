...
def parse_listing_block(block: list[dict[str, str]], state_name: str, state_code: str, page_url: str) -> Optional[dict[str, Any]]:
    ...
    name = clean_name(name_lines[0]) if name_lines and not is_notice_only(name_lines[0]) else clean_name(lines[0])
...
# Map KML Canadian province abbreviations to full names used by the scraper
_CA_PROVINCE_ABBR: dict[str, str] = {
    "AB": "Alberta", "BC": "British Columbia", "MB": "Manitoba",
    "NB": "New Brunswick", "NL": "Newfoundland", "NS": "Nova Scotia",
    "ON": "Ontario", "PE": "Prince Edward Island", "QC": "Quebec",
    "SK": "Saskatchewan", "YT": "Yukon Territory", "NT": "Northwest Territories",
}
...
def _kml_location(name: str) -> tuple[str, str, str]:
    ...
    # Canadian province full name: "Alberta, Canada - Calgary"
    m = re.match(r"^(.+?),\s*(?:Canada|Candada)\s*-\s*(.+)$", text, re.IGNORECASE)
    if m:
        province = clean(m.group(1))
        return clean(m.group(2)), f"{province}, Canada", "Canada"

    # Canadian province abbreviation: "BC - Abbotsford" or "NB - Fredericton"
    m = re.match(r"^([A-Z]{2})\s*-\s*(.+)$", text)
    if m and m.group(1) in _CA_PROVINCE_ABBR:
        province = _CA_PROVINCE_ABBR[m.group(1)]
        return clean(m.group(2)), f"{province}, Canada", "Canada"

    # International with city: "Z - United Kingdom - Folkestone"
    ...
    # International without city: "Z - United Kingdom"
